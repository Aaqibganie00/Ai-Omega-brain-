...
export type ProjectMemory = { context:string[]; decisions:string[]; requirements:string[]; unresolvedIssues:string[] }
export type Task = { id:string; projectId:string; title:string; status:TaskStatus; dependencies:string[]; assignedAgent?:string; input?:unknown; output?:unknown; error?:StructuredError; retryCount:number; createdAt:string; updatedAt:string }
export type Artifact = { id:string; name:string; kind:'file'|'report'|'log'; status:'draft'|'verified'|'failed'; createdAt:string }
export type ActivityEvent = { id:string; projectId:string; type:string; actor:string; message:string; metadata:Record<string,unknown>; createdAt:string }
