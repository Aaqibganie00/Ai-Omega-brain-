...
export class AgentRegistry { private readonly agents = new Map<string, Agent>(); register(agent: Agent): this { this.agents.set(agent.id, agent); return this } get(id: string): Agent | undefined { return this.agents.get(id) } list(): Agent[] { return [...this.agents.values()] } }
...
export function createDefaultAgentRegistry(): AgentRegistry { const registry = new AgentRegistry(); for (const role of ['planner', 'researcher', 'coder', 'tester', 'debugger', 'critic']) registry.register(new FoundationAgent(role, role, `${role} responsibilities are defined but execution is not enabled.`)); return registry }
